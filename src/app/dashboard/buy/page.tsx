...
const CATEGORIES = ["All Items", "Smart Home", "Audio", "Accessories", "Wearables"];
...
export default function BuyPage() {
  ...
  const query = useStorefront({ sellerAccountId, q: queryText }, cursor);
  ...
  return <div className="buy-page">
    ...
    <div className="buy-categories" role="tablist" aria-label="Product categories">{CATEGORIES.map((item) => <button key={item} type="button" role="tab" aria-selected={category === item} className={category === item ? "active" : ""} onClick={() => setCategory(item)}>{item}</button>)}</div>
