...
import "./product-carousel-auto.css";
import "./buy-breadcrumb.css";
...
function ProductCard({ item, sellerAccountId, onAdded }: { item: StockItemView; sellerAccountId: string; onAdded: () => void }) {
  const add = useAddToCart(sellerAccountId); const media = item.variant.product.media ?? []; const [index, setIndex] = useState(0); const [quantity, setQuantity] = useState(1); const adding = useRef(false); const price = item.discountPrice ?? item.sellPrice; const out = item.available <= 0; const low = item.available > 0 && item.available <= 5;
  useEffect(() => { if (media.length < 2) return; const timer = window.setInterval(() => setIndex((value) => (value + 1) % media.length), 3200); return () => window.clearInterval(timer); }, [media.length]);
  async function handleAdd() { if (adding.current || add.isPending) return; adding.current = true; try { await add.mutateAsync({ variantId: item.variant.id, quantity }); onAdded(); } catch { /* global feedback */ } finally { adding.current = false; } }
  const href = `/dashboard/buy/${item.id}`;
  return <article className={`buy-product-card ${out ? "out" : ""}`}><div className="buy-product-image"><Link href={href} className="buy-product-image-track" aria-label={`View details for ${item.variant.product.name}`}><div className="buy-product-image-slides" style={{ transform: `translateX(-${index * 100}%)` }}>{media.length ? media.map((image) => <img key={image.url} src={image.url} alt={image.alt ?? item.variant.product.name} />) : <div className="buy-image-placeholder">□</div>}</div></Link>{media.length > 1 && <><button type="button" className="buy-image-arrow buy-image-arrow-prev" aria-label="Previous product image" onClick={() => setIndex((value) => (value - 1 + media.length) % media.length)}>‹</button><button type="button" className="buy-image-arrow buy-image-arrow-next" aria-label="Next product image" onClick={() => setIndex((value) => (value + 1) % media.length)}>›</button><div className="buy-image-dots">{media.map((image, imageIndex) => <button type="button" key={image.url} className={imageIndex === index ? "is-active" : ""} aria-label={`Show image ${imageIndex + 1}`} onClick={() => setIndex(imageIndex)} />)}</div></>}{out ? <span className="stock-badge out">OUT OF STOCK</span> : <span className={`stock-badge ${low ? "low" : "available"}`}>{low ? `Low Stock (${item.available})` : `In Stock (${item.available})`}</span>}</div><div className="buy-product-info"><div><Link href={href} className="buy-product-title"><h2>{item.variant.product.name}</h2><span>SKU: {item.variant.sku}</span></Link><p>{item.variant.name}</p></div><div className="buy-product-bottom"><div className="buy-price"><strong>{formatMoney(price)} <em>/ unit</em></strong></div>{out ? <button type="button" disabled>Notify Me</button> : <div className="buy-actions"><div className="buy-quantity"><button type="button" disabled={quantity <= 1} onClick={() => setQuantity((value) => Math.max(1, value - 1))}>−</button><span>{quantity}</span><button type="button" disabled={quantity >= item.available} onClick={() => setQuantity((value) => Math.min(item.available, value + 1))}>+</button></div><button type="button" className="buy-add" disabled={add.isPending} onClick={handleAdd}><CartPlus className="h-4 w-4" /><b className="buy-add-label">Add</b></button></div>}</div></div></article>;
}

function LegacyProductCard({ item, sellerAccountId, onAdded }: { item: StockItemView; sellerAccountId: string; onAdded: () => void }) {
  ...
  async function handleAdd() {
    if (addingRef.current || addToCart.isPending) return;
    addingRef.current = true;
    // Success/error feedback comes from the global ToastProvider (lib/api.ts emits on every
    // non-GET call) — a second local toast here would just duplicate it.
    try { await addToCart.mutateAsync({ variantId: item.variant.id, quantity }); onAdded(); }
    catch { /* already surfaced globally */ }
