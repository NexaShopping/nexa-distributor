...
export function SalesAnalytics() {
  ...
  const sales = useSales(status ? { status } : {}, undefined); const orders = sales.data?.data.orders ?? [];
  ...
  async function exportSalesRegister() {
    ...
    setExporting(true);
    try {
      const blob = await api.download(`/orders/export?from=${exportFrom}&to=${exportTo}`);
      const url = URL.createObjectURL(blob);
      const anchor = document.createElement("a");
      anchor.href = url;
      anchor.download = `sales-register-${exportFrom}-to-${exportTo}.csv`;
      anchor.click();
